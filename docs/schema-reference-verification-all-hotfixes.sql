-- =============================================================================
-- RUN ALL REFERENCE VERIFICATION HOTFIXES (Supabase SQL Editor)
-- Fixes: ambiguous expires_at (send), ambiguous verification (confirm)
-- Drops broken function copies first, then recreates. Safe to re-run.
-- =============================================================================

drop function if exists public.complete_reference_verification(text, jsonb);
drop function if exists public.complete_reference_verification(text);
drop function if exists public.request_reference_verification(text);
