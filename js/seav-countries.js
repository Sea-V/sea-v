// /js/seav-countries.js — shared country reference data (name, ISO 3166-1
// alpha-2 code, and international dial code) used by any UI that needs a
// country picker: the profile phone number's country-code selector, the
// Nationality dropdown, and the Passports Held country picker. One list
// kept in one place so all three stay visually and alphabetically in sync.
(function () {
  "use strict";

  // name: display name shown in dropdowns.
  // iso2: ISO 3166-1 alpha-2 code, used to derive the flag emoji.
  // dial: international calling code (no leading "+"). Shared/ambiguous
  // codes (e.g. "1" for the NANP countries) are fine — the dial code is
  // only used to build/parse profile.phone, not to uniquely identify a
  // country on its own.
  const COUNTRIES = [
  ...
  // Builds a flag emoji from a two-letter ISO code using Unicode regional
  // indicator symbols (U+1F1E6 = "A") — no image assets needed, and it
  // renders natively on every modern OS/browser SEA-V targets.
  ...
  // string, which keeps the full number the user typed either way.
  const AMBIGUOUS_DIAL_PREFERENCE = { 1: "United States", 7: "Russia" };

  function getCountryByDial(dial) {
    const target = String(dial || "").trim();
    if (!target) return null;

    const preferredName = AMBIGUOUS_DIAL_PREFERENCE[target];
    if (preferredName) {
      const preferred = COUNTRIES.find((c) => c.name === preferredName);
      if (preferred) return preferred;
    }

    return COUNTRIES.find((c) => c.dial === target) || null;
  }
