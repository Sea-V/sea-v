document.addEventListener("DOMContentLoaded", () => {
  ...
  const pf_qualification = el("pf_qualification"); 
...
const previewRank = el("previewRank");
const previewQualification = el("previewQualification");
const previewNationality = el("previewNationality");
const previewDob = el("previewDob");
const previewLocation = el("previewLocation");
const previewEmail = el("previewEmail");
const previewPhone = el("previewPhone");
const previewAvailability = el("previewAvailability");
...
const previewPhoto = el("profilePreviewPhoto");
  ...
  if (existing) {
    ...
    pf_qualification.value = existing.qualification || "";
...
function renderPreview(profile) {
  ...
  if (previewRank) previewRank.textContent = profile.rank || "—";
  if (previewQualification) previewQualification.textContent = profile.qualification || "—";
  if (previewNationality) previewNationality.textContent = profile.nationality || "—";
  if (previewDob) previewDob.textContent = profile.dob || "—";
  if (previewLocation) previewLocation.textContent = profile.location || "—";
  if (previewEmail) previewEmail.textContent = profile.email || "—";
  if (previewPhone) previewPhone.textContent = profile.phone || "—";
  if (previewAvailability) previewAvailability.textContent = profile.availability || "—";
...
renderPreview(load("seav_profile", {
  name: pf_name.value,
  rank: pf_rank.value,
  qualification: pf_qualification.value,
...
form.addEventListener("input", () => {
  ...
  renderPreview({
    ...
    qualification: pf_qualification.value.trim(),
  ...
  form.addEventListener("submit", async (e) => {
    ...
    profile.qualification = pf_qualification.value.trim();
